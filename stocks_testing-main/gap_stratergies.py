...
def filled_gap_stratergy(cash, sym, k_stop_loss, k_target, str_time, end_time, buy_position=False, sell_position=False):
    ...
    if isinstance(df, pd.DataFrame):
        ...
        df = signals.decide_filled_gap(df)

        # Plots
        # fig, ax = plt.subplots(2)
        # fig.suptitle('Estrategia GAP: ' + sym + ' desde: ' + str_time + ' hasta: ' + end_time)
        # fig.tight_layout()
        #
        # x_axe = df.index
        ...
        print('Numero de compras para ' + sym + ' :' + str(compras) + '\n')
